...
LRESULT CALLBACK hookWndProcW(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SYSCOMMAND:
		...
		default:
			if (pData->m_hAdditionalMenu && FindMenuItem(syscmd, pData->m_hAdditionalMenu) != -1)
			{
				SetForegroundWindow(hVDWnd);
...
HOOKDLL_API DWORD WINAPI doHookWindow(HWND hWnd, int data)
{
	...
	pHookData->m_hAdditionalMenu = CreateMenu();
...
HOOKDLL_API DWORD WINAPI doUnHookWindow(HINSTANCE hInstance, HWND hWnd)
{
		...
		(WaitForSingleObject(pData->m_hMutex, INFINITE) != WAIT_FAILED))
	{
		...
		if (pData->m_hAdditionalMenu)
			CleanupMainMenu(hWnd, pData->m_hAdditionalMenu);
...
void CleanupMainMenu(HWND hWnd, HMENU hAdditionalMenu)
{
	HMENU hMenu = GetSystemMenu(hWnd, FALSE);
	...
	{
		//Remove the menu from the system menu of the window
		int nCount = GetMenuItemCount(hAdditionalMenu);
		for(int i = 0; i <= nCount; i++)
		{
			RemoveMenu(hMenu, 0, MF_BYPOSITION);   //delete the menu
		}
	}


	//Release resources associated with the menu
	DestroyMenu(hAdditionalMenu);
}
...
void InitPopupMenu(HWND hWnd, HMENU hMenu)
{
	...
	DWORD res;
	HANDLE hFileMapping = menuinfo.GetFileMapping();
	LRESULT lr = SendMessageTimeout(hVDWnd, WM_VD_PREPARE_HOOK_MENU, (WPARAM)hFileMapping, (LPARAM)pHookData->m_iData,
	...
	if (RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Typz Software\\Virtual Dimension\\", 0, KEY_READ, &regKey) == ERROR_SUCCESS)
	{
		if(RegQueryValueEx(regKey, "DesktopsOnMainmenu", NULL, &dwType, (BYTE*)&byteData, &dwCbData) == ERROR_SUCCESS)
		{
			...
			if(byteData == 1)
			{
				...
				DWORD res;
				HANDLE hFileMapping = menuinfo.GetFileMapping();
				LRESULT lr = SendMessageTimeout(hVDWnd, WM_VD_PREPARE_HOOK_MENU_TMP, (WPARAM)hFileMapping, (LPARAM)pHookData->m_iData,
					SMTO_ABORTIFHUNG|SMTO_NORMAL, 30000, &res);
				if (lr && res)
				{
					while(pHookData->m_hAdditionalMenu != NULL && GetMenuItemCount(pHookData->m_hAdditionalMenu) > 0)
						RemoveMenu(pHookData->m_hAdditionalMenu, 0, MF_BYPOSITION);
					...
					{
						TCHAR szItemText[20];
						memset(szItemText, 0, 20);
						MENUITEMINFO mi;
						mi.cbSize = sizeof(MENUITEMINFO);
						mi.fMask = MIIM_STRING|MIIM_ID|MIIM_STATE;
						mi.dwTypeData = szItemText;
						mi.cch = 20;
						BOOL bRet = GetMenuItemInfo(pHookData->m_hAdditionalMenu, i, TRUE, &mi);

						if (hSysMenu)
						{
							InsertMenuItem(hSysMenu, i + 2, TRUE, &mi);
						}
					}
					InsertMenu(hSysMenu, i + 2, MF_BYPOSITION | MF_SEPARATOR, 0, NULL);
				}
			}
			else
				nCount = 0;
